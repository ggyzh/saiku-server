...
//console.log("q : non empty");
    ...
    a ? (this.model().queryModel.axes.ROWS.nonEmpty = true, this.model().queryModel.axes.COLUMNS.nonEmpty = true) : (this.model().queryModel.axes.ROWS.nonEmpty = false, this.model().queryModel.axes.COLUMNS.nonEmpty = false)
...
//         a && (this._data = a, this.processData(a, b), this._hasProcessed = true)
//     };
...
//     for (; 0 < b--;)
//         if (a[b][c].properties.uniquename != a[b][c + 1].properties.uniquename) return true;
//     return false
// }
// 
// function topParentsDiffer(a, b, c) {
//     for (; 0 < c--;)
//         if (a[b][c].properties.uniquename != a[b - 1][c].properties.uniquename) return true;
//     return false
// }
/* Cognos格式 BIS */
...
/* BIS */
    ...
    var c = "",
        ...
        w = [],
        B = true;
    b && (q = b.hasOwnProperty("batchSize") ? b.batchSize : null, B = b.hasOwnProperty("wrapContent") ? b.wrapContent : true);
    var r = {};
    ...
    for (var v =
        ...
        for (var x = 0, J = f[v].length; x < J; x++) {
           ...
           /* BIS */
            ...
            if ("COLUMN_HEADER" === p.type && "null" === p.value && (null == k || x < k)){
                ...
                null == k && (k = x);
                f[v].length == x + 1 ? l = true : m = e[v][x + 1];
                if (l) "null" ==
                    /* Cognos格式 BIS */
                    ...
                    var F = 1 < x && 1 < v && !h && x > k ? e[v - 1][x + 1].value != e[v - 1][x].value || e[v - 1][x + 1].properties.uniquename != e[v - 1][x].properties.uniquename : false,
                        C = 999 < g ? true : false;
                ...
                d += '\x3cth class\x3d"row_header"\x3e' + (B ? "\x3cdiv\x3e" + p.value + "\x3c/div\x3e" : p.value) + "\x3c/th\x3e", h = true, n = x, p.properties.hasOwnProperty("dimension") && (A = p.properties. hierarchy,/* Cognos格式去同纬度多层级bug BIS */ A in s || (s[A] = []), s[A].push(p.properties.level));
            } 
             /* BIS */	
        ...
        /*Cognos分页  BIS */
        t && q ? v <= q ? (u || z || (c += "\x3c/thead\x3e\x3ctbody\x3e", z = true), c += d, 0 < g.length && (c += g)) : (w.push(d), 0 < g.length && w.push(g)) : (u || z || (c += "\x3c/thead\x3e\x3ctbody\x3e",
            z = true), c += d, 0 < g.length && (c += g))
    }
    ...
    for (var v =
        ...
        for (var x = 0, J = f[v].length; x < J; x++) {
            ...
            if ("COLUMN_HEADER" === p.type && "null" === p.value && (null == k || x < k))
                ...
                d += '\x3cth class\x3d"row_header"\x3e' + (B ? "\x3cdiv\x3e" + p.value + "\x3c/div\x3e" : p.value) + "\x3c/th\x3e", h = true, n = x, p.properties.hasOwnProperty("dimension") && (A = p.properties.dimension, A in s || (s[A] = []), s[A].push(p.properties.level));
            else if ("DATA_CELL" === p.type) {
                t = true;
                C = "";
                A = p.value;
...
//     this.cccOptions = {};
//     this.data = null;
//     this.hasRendered = this.hasProcessed = false;
...
//         visible: false
//     }), true)) : 1 < c.length && (a.keptVisibleDatumSet.splice(c.length - 1, 1), b = b.datums(null, {
//         visible: false
...
//             if (1500 < d.datums().count()) pvc.data.Data.setSelected(a, true);
...
//     b.zoom && (d = c.legend, c = _.extend(c, a), false ===
//         d && (c.legend = false));
...
//         c = false;
//     a.chart.options && a.chart.options.animate && (c = true);
//     if (!c || $(a.el).find(".canvas_wrapper").is(":visible")) $(a.el).find(".canvas_wrapper"), $(a.el).find(".canvas_wrapper").hide();
//     try {
//         c && $(a.el).find(".canvas_wrapper").show(), a.chart.render(), a.hasRendered = true
//     } catch (d) {
//         $("#canvas_" + a.id).text("Could not render chart" +
//             d)
//     }
//     if (a.chart.options && a.chart.options.animate) return false;
...
//                             B = 0 !== w,
//                             r = z.properties.raw;
//                         r && "null" !== r ? w = parseFloat(r) : "number" !== typeof z.value && parseFloat(z.value.replace(/[^a-zA-Z 0-9.]+/g, "")) && (w = parseFloat(z.value.replace(/[^a-zA-Z 0-9.]+/g, "")), B = false);
//                         0 <
...
//                     e = d
//                 }
//             c && (this.rawdata = a.data, this.data = d, this.hasProcessed = true, this.data.height = this.data.resultset.length);
//             return d
//         }
...
var MDXModal = Modal.extend({
    ...
    SelectionsModal = Modal.extend({
        ...
        initialize: function(a) {
            ...
            this.axis = "undefined";
            a.axis ? (this.axis = a.axis, "FILTER" == a.axis && (this.use_result_option = false)) : (a.target.parents(".fields_list_body").hasClass("rows") && (this.axis = "ROWS"), a.target.parents(".fields_list_body").hasClass("columns") &&
                (this.axis = "COLUMNS"), a.target.parents(".fields_list_body").hasClass("filter") && (this.axis = "FILTER", this.use_result_option = false));
            ...
            b.val("");
            1 < _.size(a.levels) && c && c.hasOwnProperty("aggregators") && c.aggregators ? (0 < c.aggregators.length && (this.show_totals_option = c.aggregators[0]), b.removeAttr("disabled")) : (b.attr("disabled", true), this.show_totals_option = "");
        ...
        populate: function(a, b) {
            ...
            $(c.el).find(".selection_options").selectable({
                ...
                stop: function(a, b) {
                    $(c.el).find(".selection_options li.ui-selected input").each(function(a, b) {
                        b && b.hasAttribute("checked") ? b.checked = true : $(b).attr("checked", true);
                        $(b).parents(".selection_options").find("li.all_options input").prop("checked", true)
                    });
            ...
            "EXCLUSION" === this.selection_type ? ($(this.el).find(".selection_type_inclusion").prop("checked", false), $(this.el).find(".selection_type_exclusion").prop("checked", true)) : ($(this.el).find(".selection_type_inclusion").prop("checked", true), $(this.el).find(".selection_type_exclusion").prop("checked", false));
            Saiku.i18n.translate();
            Saiku.ui.unblock()
        },
        ...
        click_all_member_selection: function(a, b) {
            $(a.currentTarget).find("input").is(":checked") ? $(a.currentTarget).parent().find("li.option_value input").prop("checked", true) : $(a.currentTarget).parent().find("li.option_value input").removeAttr("checked")
        },
        ...
        show_unique_action: function() {
            ...
            true === this.show_unique_option ? ($(this.el).find(".available_selections, .used_selections").addClass("unique"), $(this.el).find(".available_selections, .used_selections").removeClass("caption")) : ($(this.el).find(".available_selections, .used_selections").addClass("caption"),
...
//         add_role: function(a) {
//             a.preventDefault();
//             if ("PRIVATE" == this.acltype) return false;
...
//             $(this.el).find("input.private").is(":checked") ? ($(this.el).find(".permissions").addClass("disabled_toolbar"), $("input.acl, input.filterbox, input.add_role, input.remove_acl").prop("disabled", true), this.acltype = "PRIVATE") : ($(this.el).find(".permissions").removeClass("disabled_toolbar"), $("input.acl, input.filterbox, input.add_role, input.remove_acl").prop("disabled", false), this.acltype = "SECURED")
//         },
...
//                     }),
//                     $(b.el).find(".n").val(b.n), true === b.isMdx && null !== b.sortliteral && ($(this.el).find(".type").val("custom"), $(this.el).find(".sortingoption").html("").html("\x3ctextarea class\x3d'sortliteral'\x3e" + b.sortliteral + "\x3c/textarea\x3e")))
//             });
...
//                 c = [],
//                 c = c.concat(d, e);
//             for (; 0 < c.length;) return c[0].name === b ? true : false
//         },
//         check_len_cms: function(a) {
//             return 0 < ("calcmeasure" === a ? this.workspace.query.helper.getCalculatedMeasures() : this.workspace.query.helper.getCalculatedMembers()).length ? true : false
//         },
//         reset_form: function() {
...
//             var b = $(a.target);
//             a.preventDefault();
//             if ($(a.target).hasClass("disabled_toolbar")) return false;
...
/* BIS */	
        ...
        measure_action: function(a) {
            var b = this;
            if ("undefined" == typeof this.workspace.query || "QUERYMODEL" != this.workspace.query.model.type || "view" == Settings.MODE) return false;
            ...
            $.contextMenu({
                ...
                build: function(a, f) {
                    var g = {},
                        ...
                        l, m, n, s, q, t, u, z, w, B = false,
                        r = false,
                        D = false;
                    k && k.filters && _.each(k.filters, function(a) {
                        "N" == a.flavour && (l = a["function"], m = a.expressions[0], n = a.expressions[1], D = true); /*限制条件选中后加粗 BIS*/
                        "Generic" == a.flavour && (s = a.expressions[0], B = true)
                    });
                    k && k.sortOrder && (q = k.sortOrder, t = k.sortEvaluationLiteral, r = true);
                    ...
                    _.each(u, function(a) {
                        a.name == l && _.each(a.hierarchies, function(a) {
                            a.uniqueName == m && _.each(a.levels, function(a) {
                                ...
                                }; - 1 < _.indexOf(z, a.uniqueName) && (t[a.name].disabled = true, t["remove-" + a.name] = {
                                    name: a.caption,
                                    payload: JSON.stringify({
                                        hierarchy: m,
                                        uniquename: a.uniqueName,
                        ...
                        callback: function(a, c) {
                            ...
                            if ("keeponly" === a) {
                                ...
                                g && g.levels.hasOwnProperty(s) && (d.push({
                                    ...
                                    type: "INCLUSION",
                                    members: d
                                }, b.workspace.drop_zones.synchronize_query(), b.workspace.query.run(true))
								/* 编辑和撤销功能 BIS */	
                                //create new object to track query
                                var x = new Object;
                                x = {
...
//console.log(b.workspace.query.trackQuery);
                  ...
                  /* BIS */	
						   ...
						   if("keep" === a.substring(0, a.indexOf("-"))){
							  ...
							  b.workspace.query.helper.includeLevel(f, m, h, null), b.workspace.drop_zones.synchronize_query(), b.workspace.query.run(true));
...
//console.log(b.workspace.query.trackQuery);
                                ...
                                "include" === a.substring(0, a.indexOf("-")) && (h = a.substring(a.indexOf("-") + 1), b.workspace.query.helper.includeLevel(f, m, h, null), b.workspace.drop_zones.synchronize_query(), b.workspace.query.run(true))
                            }
                 /* BIS */	
    ...
    Workspace = Backbone.View.extend({
        ...
        initialize: function(a) {
            ...
            this.isReadOnly = "view" == Settings.MODE || false;
            a && a.item && (this.item = a.item) && this.item.hasOwnProperty("acl") && 0 > _.indexOf(this.item.acl, "WRITE") && (this.isReadOnly = true, this.viewState = "view");
...
/* BIS */	
        ...
        adjust: function() {
            var a =
                ...
                b = 87;
            if (true === Settings.PLUGIN || true === Settings.BIPLUGIN) b = 2, "table" == Settings.MODE && (b = -5);
                        ...
                        _.each(d.schemas, function(e) {
                            0 < e.cubes.length && _.each(e.cubes, function(f) {
                                if ("undefined" === typeof f.visible || f.visible) {
                                    ...
                                    a.schema === g && a.cube === f && (b.selected_cube = c.name + "/" + d.name + "/" + g + "/" + f, b.isUrlCubeNavigation = true, _.delay(b.new_query, 1E3))
                                }
                            })
                        })
                    })
        ...
        create_new_query: function(a) {
            a.query && (a.query.destroy(), a.query.clear(), a.query.name && (a.query.name = void 0, a.update_caption(true)), a.query.name =
                void 0);
            a.clear();
            a.processing.hide();
            Saiku.session.trigger("workspace:clear", {
            ...
            if (!a.selected_cube) return $(a.el).find(".calculated_measures, .addMeasure, .searchMeasure").hide(),/*指标搜索框 BIS */ $(a.el).find(".dimension_tree").html(""), $(a.el).find(".measure_tree").html(""), false;
        ...
        init_query: function(a) {
            ...
            "table" == Settings.MODE && this.query ? this.query.run(true) : ("MDX" == this.query.model.type ? (this.query.setProperty("saiku.olap.result.formatter", "flat"), $(this.el).find(".sidebar").hasClass("hide") || this.toggle_sidebar(), $(this.el).find(".workspace_fields").addClass("hide"),
                ...
                "#run_query")), this.adjust(), this.switch_view_state(this.viewState, true), $(this.el).find(".sidebar").hasClass("hide") || "chart" != Settings.MODE && "table" != Settings.MODE && "map" != Settings.MODE && "view" != Settings.MODE && !this.isReadOnly || this.toggle_sidebar(), "view" == Settings.MODE && this.query || this.isReadOnly/*用户保存权限及*编辑按钮* BIS */ && false /*BIS*/ && false ? (this.query.run(true), void 0 === this.selected_cube && (c = this.query.model.cube.schema, this.selected_cube = this.query.model.cube.connection + "/" + this.query.model.cube.catalog + "/" + ("" === c || null === c ? "null" : c) + "/" +
                ...
                workspace: this
             })) : ($(this.el).find(".calculated_measures, .addMeasure, .searchMeasure").hide(),/*指标搜索框 BIS */ $(this.el).find(".dimension_tree").html(""), $(this.el).find(".measure_tree").html("")), "undefined" != typeof a && this.query.run(true), Saiku.i18n.translate()))
			 /*指标纬度框侧边动态滚动条 BIS*/
        ...
        update_parameters: function() {
            ...
            if (a.hasOwnProperty("query") && Settings.ALLOW_PARAMETERS && "view" !== Settings.MODE && "view" !== a.viewState) {
                var b = "\x3cspan class\x3d'i18n'\x3eParameters\x3c/span\x3e: ",
                    ...
                    d = false,
                    e;
                for (e in c) d = "", c[e] && null !== c[e] && (d = c[e]), b += "\x3cb\x3e" + e + "\x3c/b\x3e \x3cinput type\x3d'text' placeholder\x3d'" + e + "' value\x3d'" + d + "' /\x3e", d = true;
                b += "";
...
//                             d = a.queries[d];
//                         "undefined" != typeof d.acl && 0 > _.indexOf(d.acl, "WRITE") ? (b.commands["delete"].disabled = true, b.items["delete"].disabled = true, b.commands.edit.disabled = true, b.items.edit.disabled = true, b.commands.move.disabled = true, b.items.move.disabled = true) : (b.commands["delete"].disabled = false, b.items["delete"].disabled = false, b.commands.edit.disabled = false, b.items.edit.disabled = false, b.commands.move.disabled = false, b.items.move.disabled = false);
//                         $(this).hasClass("folder_row") ?
//                             (b.commands.open.disabled = true, b.items.open.disabled = true) : (b.commands.open.disabled = false, b.items.open.disabled = false)
//                     }
//                 },
...
//                 openQuery: this
//             });
//             Settings.REPOSITORY_LAZY && this.$el.find(".search_file").prop("disabled", true);
//             return this
//         },
//         initialize: function(a) {
...
//                             name: c,
//                             folder: ""
//                         }), b.query.trigger("query:save"), b.copy_to_repository(), a.stopPropagation(), a.preventDefault(), false
//                     }
...
//             this.content.render();
//             $(this.el).html(this.template());
//             false === this.close && ($(this.el).find(".close_tab").hide(), $(this.el).css("padding-right", "10px"));
//             var b = {
//                 "new": {
//                     name: "New",
//                     i18n: true
...
/* 编辑和撤销功能 BIS */					
   ...
   /* BIS */	
        ...
        run: function(a, b) {
            var c = this;
            Saiku.ui.unblock();
            if ("undefined" == typeof this.model.properties || false !== this.model.properties["saiku.olap.query.automatic_execution"] || false !== a && void 0 !== a && null !== a) {
                this.workspace.unblock();
                ...
                "OLAP" == f.queryType && ("QUERYMODEL" == f.type ? (g = 0 < Object.keys(f.queryModel.axes.COLUMNS.hierarchies).length, h = 0 < Object.keys(f.queryModel.axes.ROWS.hierarchies).length, k = "COLUMNS" == f.queryModel.details.axis && 0 < f.queryModel.details.measures.length, h && g &&
                    k || (e = ""), g || k || (e += '\x3cspan class\x3d"i18n"\x3e你需要在列中放入至少一个维度或层级以生成有效查询...\x3c/span\x3e'), h || (e += '\x3cspan class\x3d"i18n"\x3e你需要在行中放入至少一个层级以生成有效查询...\x3c/span\x3e'), (g || k) && h && (d = true)) : "MDX" == f.type && ((d = f.mdx && 0 < f.mdx.length) || (e = '\x3cspan class\x3d"i18n"\x3eYou need to enter some MDX statement to execute.\x3c/span\x3e')));
...
/*        process_datasources: function(a, b) {
            ...
            if (this.initialized) Settings.INITIAL_QUERY ||
                ...
                if (!_.has(c, "splash")) c.splash = true;
                else if (_.has(c, "splash") && c.splash || _.has(c, "splash") && null === c.splash) c.splash = true;
                    ...
                    for (var f = c.catalogs[d], g = 0, h = f.schemas.length; g < h; g++)
                        for (var k = f.schemas[g], l = 0, m = k.cubes.length; l < m; l++) {
                            ...
                            "undefined" !== typeof localStorage && localStorage && null !== localStorage.getItem("cube." + n) ? this.cube[n] = new Cube(JSON.parse(localStorage.getItem("cube." + n))) : (this.cube[n] = new Cube({
                                key: n
                            }), true === Settings.DIMENSION_PREFETCH && this.cube[n].fetch())
                        }!this.initialized && Backbone.history && (Backbone.history.start(), this.initialized = true)
        },
        url: function() {
            return this.first ? (this.first = false, encodeURI(Saiku.session.username + "/discover")) : encodeURI(Saiku.session.username + "/discover/refresh")
        }
    }), */
        ...
        // URLParams: {
            ...
            // equals: function() {
                // var a = Array.prototype.slice.call(arguments),
                    // b = this.paramsURI();
                // return _.isEqual(b, a[0]) ? true : false
            // }
        // },
...
// var DateFilterModal = Modal.extend({
        ...
        // check_saikuDayFormatString: function() {
            ...
            // this.$el.find(".selection-radio").each(function(b, c) {
                // _.find(a.dataLevels, function(b) {
                    // a.name === b.name && b.saikuDayFormatString && $(c).prop("disabled", false)
                // })
            // })
        // },
        ...
        // disable_divselections: function(a) {
            ...
            // this.$el.find(".available-selections *").prop("disabled", true).off("click");
            // c.closest(".box-selections").find(".available-selections").attr("available", true);
            // c.closest(".box-selections").find(".available-selections *:not(.keep-disabled)").prop("disabled", false).on("click");
        ...
        // populate: function() {
            ...
            // if (a && !_.isEmpty(a))
                // if ("operator" === a.type) {
                    ...
                    // b.prop("checked", true);
                    // c.prop("checked", true);
                    // this.disable_divselections(b, true);
                    // this.show_fields(c);
                    ...
                    // })
                // } else "fixed-date" === a.type ? (b = this.$el.find("#selection-radio-fixed-date"), b.prop("checked", true), this.$el.find("#" + a.checked).prop("checked", true)) : (b = this.$el.find("#selection-radio-available"), b.prop("checked", true), this.$el.find("#date-input").val(a.periodAmount),
                    // this.$el.find('select#period-select option[id\x3d"' + a.periodSelect + '"]').prop("selected", true)), this.disable_divselections(b, true)
        // },
                ...
                // if ("true" === $(k).attr("available")) {
                    ...
                    // "operator" === $(k).attr("selection-name") ? $(k).find("input:radio").each(function(a, e) {
                        ...
                        // }) : "fixed-date" === $(k).attr("selection-name") ? $(k).find("input:radio").each(function(a, b) {
                            // true === $(b).is(":checked") && (c = $(b).attr("id").split("-")[1], e = $(b).val(), h.checked = $(b).attr("id"))
                        // }) : "rolling-date" ===
        ...
        // show_button_clear: function() {
            ...
            // return b && !_.isEmpty(b) ? a.get(c) ? true : false : false
        // },
        // finished: function(a) {
            // this.$el.dialog("destroy").remove();
            // a || this.query.run()
...
// Backbone.sync = function(a, b, c) {
    ...
    // b = {
        ...
        // async: a
    // };
    // false === c.processData && (b.processData = false);
